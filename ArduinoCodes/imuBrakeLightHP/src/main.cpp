...
// ---------- Brake intensity (direction-agnostic) ----------
...
void setBrakeBar(float level01, bool hardFlash) {
  ...
  uint8_t brakeR = hardFlash ? BRAKE_R_MAX : (uint8_t)(60 + level01 * 195); // 60..255
...
void loop() {
  ...
  if (now - lastImu >= IMU_MS) {
    ...
    if (IMU.accelerationAvailable()) {
      ...
      // Direction-agnostic "braking intensity"
  ...
  if (now - lastLed >= LED_MS) {
    ...
    if (hardFlash) {
      ...
      onboardRGB(0, 255, 255);  // onboard red (active-low)
    } else {
      setBrakeBar(brakeLevel, false);
      if (brakeLevel > 0.05f) {
        onboardRGB(80, 255, 255);  // dim onboard red hint
      } else if (moving) {
        onboardRGB(255, 0, 255);   // green = moving
      } else {
        onboardRGB(255, 255, 0);   // blue = idle
      }
    }
  }
}
